...
def get_pelee_ssd(name, base_size, features, filters, sizes, ratios, steps, classes,
            dataset, pretrained=False, pretrained_base=True, ctx=mx.cpu(),
            root=os.path.join('~', '.mxnet', 'models'), **kwargs):
    pretrained_base = False if pretrained else pretrained_base
    base_name = None if callable(features) else name
    net = Pelee_SSD(base_name, base_size, features, filters, sizes, ratios, steps,
...
def ssd_224_peleenet50_voc(pretrained=False, pretrained_base=False, **kwargs):
    classes = VOCDetection.CLASSES
    return get_pelee_ssd('peleenet', 224,
                   features=['stage3_relu40_fwd', 'stage4_relu30_fwd'],
                   filters=[128,],
                   sizes=[51.2, 102.4, 189.4],
                   ratios=[[1, 2, 0.5, 3, 1.0 / 3]] * 2 ,
                   steps=[16, 32,],
                   classes=classes, dataset='voc', pretrained=pretrained,
                   pretrained_base=pretrained_base, **kwargs)
...
def ssd_300_peleenet50_voc(pretrained=False, pretrained_base=False, **kwargs):
    classes = VOCDetection.CLASSES
    return get_pelee_ssd('peleenet', 300,
                   features=['stage2_relu20_fwd', 'stage4_relu30_fwd'],
                   filters=[256, 256, 256],
                   sizes=[21, 45, 99, 153, 207, 261],
                   ratios=[[1, 2, 0.5]] + [[1, 2, 0.5, 3, 1.0 / 3]] * 3 + [[1, 2, 0.5]],
                   steps=[16, 32, 64, 128, 300],
                   classes=classes, dataset='voc', pretrained=pretrained,
                   pretrained_base=pretrained_base, **kwargs)

def ssd_512_fpn_resnet18_v1_voc(pretrained=False, pretrained_base=True, **kwargs):
    classes = VOCDetection.CLASSES
    from ..resnet import resnet18_v1
    base_network = resnet18_v1(pretrained=pretrained_base,**kwargs)
    return get_fpn_ssd(base_network, 512,
                   features=['stage2_activation1','stage3_activation1', 'stage4_activation1'],
                   filters=[],
                   fpn_filters = [256, 256, 256],
